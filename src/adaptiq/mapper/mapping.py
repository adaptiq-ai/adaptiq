...
class AdaptiqStateMapper:
    ...
    def __init__(
        ...
        if provider == "openai":
            self.reconciliation_llm = ChatOpenAI(
                model=llm_model_name_for_reconciliation, api_key=llm_api_key
            )
        ...
        self.classification_prompt_template = ChatPromptTemplate.from_template(
    ...
    def _invoke_llm_for_classification(
        self, input_state: Union[str, List, Dict]
    ) -> Dict:
        """
    ...
    def save_classification_json(
        self, classification_results: List[Dict], output_path: str
    ) -> None:
        """
    ...
    def from_qtable_file(
        qtable_file_path: str, llm_model_name: str, llm_api_key: str, provider: str
    ) -> "AdaptiqStateMapper":
        """
