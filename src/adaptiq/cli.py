...
def find_and_clear_log_files(search_directory=".", log_filename="log.txt", json_filename="log.json"):
    """
...
def execute_pre_run_only(args, logger):
    ...
    try:
        ...
        logger.info(f"tokens input: {input_tokens}, output: {output_tokens}, total calls: {total_calls}")
        logger.info(f"Execution time: {execution_time:.2f} seconds, Peak memory usage: {peak_memory:.2f} MB")
        ...
        aggregator._last_original_prompt = ""  # You might want to extract this from config
...
def execute_post_run_and_reconciliation(args, logger, run_number=None, crew_metrics=None, should_send_report=True):
    ...
    if crew_metrics:
        logger.info(f"{run_prefix}Processing {len(crew_metrics)} crew metrics entries...")
        ...
        for i, metrics in enumerate(crew_metrics):
            ...
            total_peak_memory = max(total_peak_memory, peak_memory_mb)  # Use max for peak memory
            total_prompt_tokens += prompt_tokens
            total_completion_tokens += completion_tokens
            total_successful_requests += successful_requests
            
            logger.info(f"{run_prefix}Metrics {i+1}/{len(crew_metrics)}: "
    ...
    try:
        ...
        logger.info(f"{run_prefix}Agent token stats: input={total_prompt_tokens}, output={total_completion_tokens}, calls={total_successful_requests}")
        logger.info(f"{run_prefix}Total execution time: {total_execution_time:.2f} seconds, Peak memory usage: {total_peak_memory:.2f} MB")

        validated_logs = post_run_results.get("outputs", {}).get("validation_summary_path", [])
        ...
        if crew_metrics:
            for i, metrics in enumerate(crew_metrics):
                ...
                # Calculate average reward from simulated scenarios
                avg_reward = aggregator.calculate_avg_reward(
                    validation_summary_path=validated_logs, 
                ...
                logger.info(f"{run_prefix}Added run summary for execution {execution_count}")
        else:
            # Handle case where no crew metrics provided - add single summary
            logger.info(f"{run_prefix}No crew metrics provided, adding single run summary...")
        ...
        if should_send_report:
            logger.info(f"{run_prefix}Building and sending comprehensive project results...")
            ...
            if aggregator.email != "":
                ...
                if success:
                    logger.info(f"{run_prefix}Successfully sent comprehensive run results to reporting endpoint")
                else:
                    logger.warning(f"{run_prefix}Failed to send run results to reporting endpoint")
            else:
                logger.info(f"{run_prefix} are successfully saved locally")
        else:
            logger.info(f"{run_prefix}Run summaries added to aggregator - report will be sent when all runs complete")
        ...
        if should_send_report:
            logger.info(f"{run_prefix}Building and sending project results for failed run...")
            project_result = aggregator.build_project_result()
            aggregator.send_run_results(project_result)
        else:
            logger.info(f"{run_prefix}Failed run summary added to aggregator - report will be sent when all runs complete")
    ...
    if hasattr(args, 'crew_metrics') and args.crew_metrics:
        ...
        try:
            ...
            if crew_metrics_list and len(crew_metrics_list) > 0:
                current_execution_count = crew_metrics_list[-1].get('execution_count', 0)
                logger.info(f"Current execution count from crew metrics: {current_execution_count}")
...
def handle_wizard_headless_command(args):
    ...
    logger.info(f"Starting Adaptiq Wizard in headless mode with LLM provider: {args.llm_provider}")
...
def main():
    ...
    parser_default_run = subparsers.add_parser(
        "default-run",
        help="Run only the pre_run pipeline (executes once)."
    )
    ...
    parser_wizard = subparsers.add_parser(
        "wizard",
        help="Start the interactive Adaptiq wizard assistant."
    )
