...
class AdaptiqLogger(logging.Handler):
    ...
    - Allow users to register custom callbacks that are invoked on every new log.
    - Accept structured payloads via the `extra={"payload": ...}` logging argument.

    Usage:
        handler = AdaptiqLogger.setup(level=logging.INFO)

        def my_callback(log_entry):
            print("Received log:", log_entry)

        handler.add_callback(my_callback)

        logging.info("Something happened", extra={"payload": {"event": "startup"}})
        ...
        try:
            ...
            for callback in self._callbacks:
                try:
                    ...
                    # If a user callback throws, log it and move on
                    logging.getLogger(__name__).warning(
                        f"AdaptiqLogger callback error: {e}"
                    )
    ...
    def get_logs_by_type(self, log_type: str) -> List[Dict[str, Any]]:
        """
        Returns logs filtered by type (INFO, DEBUG, ERROR, WARNING).
        """
        with self._lock:
            return [
    ...
    @classmethod
    def setup(cls, level=logging.INFO) -> "AdaptiqLogger":
        """
