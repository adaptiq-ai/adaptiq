...
class ScenarioSimulator:
    ...
    def __init__(
        ...
        self.scenario_generation_prompt_template = ChatPromptTemplate.from_template(
            ...
            - "next_state_components": A string representation of a 4-element tuple in this EXACT format:  
            "({next_subtask}, simulated_action, outcome_type, context)"
            WHERE ALL ELEMENTS MUST BE STRINGS, NOT DICTIONARIES OR OBJECTS
            - "key_context_changes": (Dictionary describing key changes in context after the outcome)

            CRITICAL FORMAT REQUIREMENTS:
            - The "next_state_components" must be a STRING representation of a tuple, not an actual tuple
            - ALL 4 elements in the tuple must be simple strings, never dictionaries or complex objects
            - If context involves file paths or complex data, convert to a simple string description
            - Example: "('InformationRetrieval_Company', 'search_web', 'Success', 'company background research')"
            - NOT: "('InformationRetrieval_Company', 'search_web', 'Success', {{'image_path': 'local image file'}})"
...
#                 all_simulated_steps.append(ScenarioModel(**simulated_step))
