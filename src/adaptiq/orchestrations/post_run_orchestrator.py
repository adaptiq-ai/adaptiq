...
class AdaptiqPostRunOrchestrator:
    ...
    def __init__(
        ...
        self.validation_summary_path = os.path.join(output_dir, "validation_summary.json")
    ...
    def get_agent_trace(self) -> str:
        ...
        if not trace_output:
            ...
            self.logger.info(f"Agent trace retrieval completed, captured {len(trace_output)} characters of trace")
    ...
    def parse_logs(self, raw_logs: Optional[str] = None) -> Dict[Tuple[Tuple[str, str, str, str], str], float]:
        """
        ...
        if raw_logs is None:
            if not os.path.exists(self.raw_logs_path):
                raise FileNotFoundError(f"Raw logs file not found at {self.raw_logs_path}")
        ...
        try:
            ...
            self.log_parser = AdaptiqLogParser(temp_raw_logs_path, self.parsed_logs_path)
            parsed_data = self.log_parser.parse_logs()
            
            self.logger.info(f"Log parsing completed, generated {len(parsed_data)} state-action-reward mappings")
    ...
    def validate_parsed_logs(self, raw_logs: Optional[List[Dict[str, Any]]] = None, 
                           parsed_logs: Optional[List[Dict[str, Any]]] = None) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        """
        ...
        if parsed_logs is None:
            if not os.path.exists(self.parsed_logs_path):
                raise FileNotFoundError(f"Parsed logs file not found at {self.parsed_logs_path}")
        ...
        try:
            ...
            self.logger.info(f"Validation summary saved to {self.validation_summary_path}")
    ...
    def run_full_pipeline(self) -> Dict[str, Any]:
        ...
        if self.validate_results:
            ...
            corrected_logs, validation_results = self.validate_parsed_logs(raw_logs, parsed_logs)
        ...
        pipeline_results = {
            ...
            "stats": {
                "raw_log_size": os.path.getsize(self.raw_logs_path) if os.path.exists(self.raw_logs_path) else 0,
                "parsed_entries_count": len(parsed_data) if parsed_data else 0,
            }
        }
        
        if self.validate_results:
            pipeline_results["outputs"]["validated_logs_path"] = self.validated_logs_path
            pipeline_results["outputs"]["validation_summary_path"] = self.validation_summary_path
            
            if validation_results and "summary" in validation_results:
                pipeline_results["stats"]["validation_summary"] = validation_results["summary"]
