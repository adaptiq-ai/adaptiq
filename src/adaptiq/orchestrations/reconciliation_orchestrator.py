...
from adaptiq.utils.reconciliation_utils import AdaptiqQtablePostrunUpdate, AdaptiqPromptEngineer

logger = logging.getLogger(__name__)


class AdaptiqReconciliationOrchestrator:
    """
    Orchestrator class that coordinates the entire Adaptiq reconciliation pipeline.
    
    This class manages the flow between:
    1. AdaptiqStateActionExtractor - processes execution data
    2. AdaptiqStateMapper - matches states with Q-table
    ...
    def _initialize_extractor(self):
        """Initialize the AdaptiqStateActionExtractor if not already done."""
        if self.extractor is None:
            self.extractor = AdaptiqStateActionExtractor(
                provider=self.extractor_params["provider"],
                model=self.extractor_params["model"],
                api_key=self.extractor_params["api_key"]
            )
            logger.info("AdaptiqStateActionExtractor initialized")
    
    def _initialize_mapper(self, warmed_qtable_data: Dict):
        """Initialize the AdaptiqStateMapper if not already done."""
        if self.mapper is None:
            self.mapper = AdaptiqStateMapper(
                warmed_qtable_data=warmed_qtable_data,
                provider=self.mapper_params["provider"],
                llm_model_name_for_reconciliation=self.mapper_params["llm_model_name_for_reconciliation"],
                llm_api_key=self.mapper_params["llm_api_key"]
            )
            logger.info("AdaptiqStateMapper initialized")
    
    def _initialize_updater(self):
        """Initialize the AdaptiqQtablePostrunUpdate if not already done."""
        if self.updater is None:
            self.updater = AdaptiqQtablePostrunUpdate(
                provider=self.updater_params["provider"],
                api_key=self.updater_params["api_key"],
                model=self.updater_params["model"],
                alpha=self.updater_params["alpha"],
                gamma=self.updater_params["gamma"],
                similarity_threshold=self.updater_params["similarity_threshold"]
            )
    ...
    def run_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            known_states_count = sum(1 for c in state_classifications 
                                   if c["classification"]["is_known_state"])
            logger.info(f"Found {known_states_count} known states out of {len(state_classifications)} total")
            
            # Step 4: Update Q-table based on classifications and rewards
            logger.info("Step 4: Updating Q-table")
            self._initialize_updater()
            updated_qtable = self.updater.process_data(
...
def adaptiq_reconciliation_pipeline(config_path: str, output_path: str) -> Any:
    ...
    reconciliation_orchestrator = AdaptiqReconciliationOrchestrator(
    ...
    reconciliation_orchestrator.save_results(results=result, output_file=str(results_file))
