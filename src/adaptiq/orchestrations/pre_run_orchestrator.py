...
from adaptiq.utils.pre_run_utils import AdaptiqHypotheticalStateGenerator, AdaptiqPromptConsulting, AdaptiqScenarioSimulator
from adaptiq.q_learning.q_learning import AdaptiqOfflineLearner

from adaptiq.instrumental.instrumental import instrumental_track_tokens, capture_llm_response
...
class AdaptiqPreRunOrchestrator:
    ...
    def __init__(self, config_path: str, output_path: str):
        ...
        tools_config = self.config.get('agent_modifiable_config', {}).get('agent_tools', [])
        self.agent_tools = [
            {tool['name']: tool['description']} 
            for tool in tools_config
        ] if tools_config else []

        self.tool_strings = [f"{name}: {desc}" for tool_dict in self.agent_tools for name, desc in tool_dict.items()]
    ...
    def _ensure_output_directory(self) -> str:
        ...
        if not self.output_path:
            ...
            self.output_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "results")
    ...
    def run_prompt_parsing(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"Prompt Parsing complete. Identified {len(self.parsed_steps)} steps.")
    ...
    def run_hypothetical_representation(self) -> List[Dict]:
        ...
        if not self.parsed_steps:
            self.logger.warning("No parsed steps available. Running prompt parsing first.")
        ...
        try:
            ...
            self.hypothetical_states = self.state_generator.clean_representation(raw_states)
           
            self.logger.info(f"Hypothetical State Generation complete. Generated {len(self.hypothetical_states)} state-action pairs.")
    ...
    def run_simulation(self) -> List[Dict]:
        ...
        if not self.hypothetical_states:
            self.logger.warning("No hypothetical states available. Running hypothetical representation first.")
            self.run_hypothetical_representation()
        
        try:
            # Create a filename for the simulation results
            output_dir = self._ensure_output_directory()
            simulation_output_path = os.path.join(output_dir, "adaptiq_simulated_scenarios.json")
            ...
            self.simulated_scenarios = self.scenario_simulator.generate_simulated_scenarios()
            
            self.logger.info(f"Scenario Simulation complete. Generated {len(self.simulated_scenarios)} scenarios.")
    ...
    def run_qtable_initialization(self, alpha: float = 0.8, gamma: float = 0.8) -> Dict:
        ...
        for scenario in self.simulated_scenarios:
            try:
                ...
                next_state = str(scenario.get('next_state'))  # use string representation directly

                def ensure_tuple(s):
                    if isinstance(s, str) and s.startswith("(") and s.endswith(")"):
                        try:
                            return eval(s)  # safer if you trust input; otherwise parse manually
                        except Exception:
                            return s
                    return s
        ...
        self.logger.info(f"Q-table initialized with {len(self.offline_learner.Q_table)} entries.")

        output_dir = self._ensure_output_directory()
        q_table_path = os.path.join(output_dir, "adaptiq_q_table.json")
        save_success = self.offline_learner.save_q_table(file_path=q_table_path, prefix_version="pre_run")
    ...
    def run_prompt_analysis(self) -> Dict:
        ...
        try:
            ...
            self.prompt_analysis = self.prompt_consultant.get_formatted_analysis(raw_analysis)
    ...
    def get_status_summary(self) -> Dict:
        ...
        return {
            ...
            "scenario_simulation": {
                "completed": hasattr(self, 'simulated_scenarios') and len(self.simulated_scenarios) > 0,
                "scenarios_generated": len(self.simulated_scenarios) if hasattr(self, 'simulated_scenarios') else 0
            },
            "qtable_initialization": {
                "completed": self.offline_learner is not None and len(self.offline_learner.Q_table) > 0,
                "q_entries": len(self.offline_learner.Q_table) if self.offline_learner else 0
            },
            "prompt_analysis": {
                "completed": bool(self.prompt_analysis),
                "weaknesses_found": len(self.prompt_analysis.get('weaknesses', [])) if self.prompt_analysis else 0,
                "suggestions_provided": len(self.prompt_analysis.get('suggested_modifications', [])) if self.prompt_analysis else 0
            }
        }
    ...
    def generate_estimated_prompt(self) -> str:
        ...
        if missing_components:
            self.logger.warning("Missing results from: %s. Running complete pipeline...", ', '.join(missing_components))
        ...
        try:            
            ...
            if self.offline_learner and hasattr(self.offline_learner, "Q_table"):
                for i, hypothetical_step in enumerate(self.hypothetical_states[:20]):  # Sample from first 20 states
                    ...
                    try:
                        import ast
                        if isinstance(state_repr, str) and state_repr.strip().startswith('(') and state_repr.strip().endswith(')'):
                    ...
                    if (state_key, action) in self.offline_learner.Q_table:
                        ...
                        if q_value < 0:
                            if "undeclared_tool" in action.lower() or not any(tool in action for tool in self.config.get('agent_modifiable_config', {}).get('agent_tools', [])):
                                heuristic_counts["undeclared_tool_penalty"] = heuristic_counts.get("undeclared_tool_penalty", 0) + 1
                            if "unknown" in action.lower():
                                heuristic_counts["ambiguous_action_penalty"] = heuristic_counts.get("ambiguous_action_penalty", 0) + 1
                        elif q_value > 0:
                            if isinstance(state_key, tuple) and len(state_key) >= 2:
                                if (state_key[1] == "None" or not state_key[1]) and any(x in str(state_key[0]) for x in ["Information", "Initial", "Query"]):
                                    heuristic_counts["good_first_step_reward"] = heuristic_counts.get("good_first_step_reward", 0) + 1
            ...
            if strengths:
                ...
                for i, strength in enumerate(strengths[:3]):  # Limit to first 3 for brevity
                    prompt_analysis_summary += f"- {strength}\n"
            
            if weaknesses:
                prompt_analysis_summary += "\nWeaknesses:\n"
                for i, weakness in enumerate(weaknesses[:3]):  # Limit to first 3 for brevity
                    prompt_analysis_summary += f"- {weakness}\n"
            
            if suggestions:
                prompt_analysis_summary += "\nSuggested Modifications:\n"
                for i, suggestion in enumerate(suggestions[:3]):  # Limit to first 3 for brevity
            ...
            formatted_prompt = prompt.format(
                ...
                key_heuristics=", ".join(key_heuristics) if key_heuristics else "None identified",
            ...
            output_dir = self._ensure_output_directory()
            report_path = os.path.join(output_dir, "adaptiq_analysis_pre_run_report.txt")
