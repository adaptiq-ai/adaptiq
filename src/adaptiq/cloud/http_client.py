...
class HTTPClient:
    ...
    def get(self, endpoint: str, params: Optional[Dict[str, Any]] = None, 
        ...
        try:
            ...
            return {"status_code": getattr(response, 'status_code', None) if 'response' in locals() else None, "error": str(e)}
    ...
    def patch(self, endpoint: str, 
        ...
        url = urljoin(self.base_url, endpoint)
        try:
            response = self.session.patch(url, params=params, json=json_data, headers=headers)
            response.raise_for_status()
            return {"status_code": response.status_code, "data": response.json()}
        except requests.exceptions.RequestException as e:
            return {"status_code": getattr(response, 'status_code', None) if 'response' in locals() else None, "error": str(e)}
        
    def put(self, endpoint: str, params: Optional[Dict[str, Any]] = None,
        json_data: Optional[Dict[str, Any]] = None,
        headers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
