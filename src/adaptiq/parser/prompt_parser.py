...
class AdaptiqPromptParser:
    ...
    def __init__(self, config_path: str):
        ...
        self.llm_model_name = self.adaptiq_config.get('llm_config', {}).get('model_name')
        ...
        self.agent_task_description_path = self.adaptiq_config.get('agent_modifiable_config', {}).get('prompt_configuration_file_path')
        if not self.agent_task_description_path:
            raise ValueError("Agent task description path not found in configuration file")
        
        # Get the list of tools available to the agent
        tools_config = self.adaptiq_config.get('agent_modifiable_config', {}).get('agent_tools', [])
        self.agent_tools = [
        ...
        api_key = self.adaptiq_config.get('llm_config', {}).get('api_key') or os.getenv("OPENAI_API_KEY")
        if not api_key:
            raise ValueError("OpenAI API key not found in configuration or environment")
        
        if self.provider == "openai":
            self.prompt_parser_llm = ChatOpenAI(temperature=0.0, model=self.llm_model_name, api_key=api_key)
        else:
    ...
    def _load_agent_task_description(self) -> str:
        ...
        try:
            ...
            agent_name = self.adaptiq_config.get('agent_modifiable_config', {}).get('agent_name')
    ...
    def _construct_llm_prompt_for_parsing(self, task_description_text: str) -> Dict[str, Any]:
        """
        ...
        """
        tool_strings = [f"{name}: {desc}" for tool_dict in self.agent_tools for name, desc in tool_dict.items()]
        return {
