...
class AdaptiqLogParser:
    ...
    REWARD_TOOL_SUCCESS_EMPTY_RESULT = 0.25  # Tool worked, but result was empty (e.g., "", [], {})
    PENALTY_TOOL_ERROR = -1.0
    PENALTY_TOOL_NO_RESULT_FIELD = -0.75  # Tool was called, but 'result' key is missing
    PENALTY_TOOL_NAME_EMPTY_STRING = -1.0  # If 'tool' field is an empty string in AgentAction
    
    # AgentAction: Thinking (No Tool)
    REWARD_AGENT_THINK_ACTION_GOOD_THOUGHT = 0.3  # For AgentAction with no tool and good thought
    PENALTY_AGENT_THINK_ACTION_POOR_THOUGHT = -0.3  # For AgentAction with no tool and poor thought
    ...
    PENALTY_TASKLOG_RAW_CONTAINS_ERROR = -0.75  # If raw output in TaskLog indicates an error
    
    # Keywords
    ERROR_KEYWORDS = ["error:", "traceback:", "failed to execute", "exception:", "failure:"]
    PLACEHOLDER_STRINGS_LOWER = ["none", "n/a", "missing thought", "empty thought", "task log content", "null"]
    ...
    def load_json_data(self) -> Union[Dict, List[Dict[str, Any]]]:
        ...
        with open(self.logs_path, 'r', encoding='utf-8') as file:
            try:
                ...
                raise json.JSONDecodeError(f"Invalid JSON in file '{self.logs_path}': {e.msg}", e.doc, e.pos)
        ...
        if isinstance(data, dict):
            ...
            raise ValueError("JSON content must be either a dictionary or a list of dictionaries.")
    ...
    def parse_logs(self) -> Dict[str, List[Dict]]:
        ...
        for i, log_entry in enumerate(logs):
            ...
            if entry_type == "AgentAction" or entry_type == "AgentFinish":
                ...
                if not current_thought or self.is_string_effectively_empty_or_placeholder(current_thought):
                    current_thought = "Empty thought"
                    reward += self.PENALTY_POOR_THOUGHT
                elif len(str(current_thought).strip()) < self.MIN_MEANINGFUL_THOUGHT_LEN:
                    reward += self.PENALTY_POOR_THOUGHT
                else:
            ...
            if entry_type == "AgentAction":
                ...
                if isinstance(tool_name, str) and tool_name.strip():
                    ...
                    if tool_result is not None:
                        ...
                        is_error = any(err_keyword in result_lower for err_keyword in self.ERROR_KEYWORDS)
                        
                        if is_error:
                            reward += self.PENALTY_TOOL_ERROR
                        elif not result_lower or result_lower == "[]" or result_lower == "{}":
            ...
            # --- AgentFinish Processing ---
                ...
                current_outcome = str(final_output).strip() if final_output is not None else ""
            ...
            # --- TaskLog Processing ---
                ...
                if not current_outcome:
                    ...
                    if any(err_keyword in current_outcome.lower() for err_keyword in self.ERROR_KEYWORDS):
