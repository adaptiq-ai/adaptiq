        ...
        def wrapper(*args, **kwargs):
            ...
            if include_crew_metrics:
                try:
                    ...
                    print(
                        "[INSTRUMENT] Warning: Could not import get_crew_metrics function"
                    )
                    crew_metrics = None
            ...
            if trigger_command:
                ...
                if output_path is None:
                    print(
                        "[INSTRUMENT] Warning: No 'results' folder found. Skipping AdaptiQ pipeline execution."
                    )
                    return result
                ...
                try:
                    ...
                    print(
                        f"[INSTRUMENT] Alert mode detected: {alert_mode_info['mode']}"
                    )
                    if alert_mode_info["runs"]:
                        print(
                            f"[INSTRUMENT] Number of runs configured: {alert_mode_info['runs']}"
                        )
                except Exception as e:
                    print(
                        f"[INSTRUMENT] Warning: Could not read alert mode configuration: {str(e)}"
                    )
                    alert_mode_info = {"mode": "none", "runs": None}

                # Determine if we should send report
                should_send_report = determine_should_send_report(
                    crew_metrics, alert_mode_info
                )
                ...
                if crew_metrics:
                    ...
                    print(
                        f"[INSTRUMENT] Crew metrics added to command args (size: {len(crew_metrics_json)} chars)"
                    )
                ...
                if feedback:
                    ...
                    print(
                        f"[INSTRUMENT] Feedback added to command args: {feedback[:100]}{'...' if len(feedback) > 100 else ''}"
                    )
                ...
                print(
                    f"[INSTRUMENT] Triggering command: {' '.join(cmd_args[:4])}{'...' if len(cmd_args) > 4 else ''}"
                )
                ...
                try:
                    ...
                    if return_code == 0:
                        ...
                        print(
                            f"[INSTRUMENT] AdaptiQ pipeline failed with return code: {return_code}"
                        )
                        return None

                except FileNotFoundError:
                    print(
                        "[INSTRUMENT] Error: 'adaptiq' command not found. Make sure AdaptiQ is installed and in PATH."
                    )
                    return None

                except subprocess.TimeoutExpired:
                    print(
                        "[INSTRUMENT] Error: AdaptiQ pipeline timed out after 10 minutes"
                    )
                    try:
                        process.kill()
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if provider_lower != "openai":
                print(
                    f"Warning: Provider '{provider}' not supported for token tracking. Only 'openai' is supported."
                )
            ...
            with ResponseCapture() as capture:
                ...
                if total_tokens > 0:
                    print(
                        f"✅ {func.__name__} [{mode}]: {total_tokens} tokens ({total_input_tokens} in, {total_output_tokens} out) from {len(capture.responses)} LLM calls"
                    )
                else:
                    print(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if provider_lower not in ["openai"]:
                print(
                    f"Warning: Provider '{provider}' not supported for embeddings tracking. Supported: openai"
                )
                return func(*args, **kwargs)
            ...
            def create_embed_wrapper(original_method, obj_name="embeddings"):
                def embed_wrapper(*embed_args, **embed_kwargs):
                    ...
                    embedding_call_info = {
                        ...
                        "input_text_length": (
                            len(str(embed_args[0])) if embed_args else 0
                        ),
                    ...
                    if hasattr(attr_value, "embed_query"):
                        ...
                        try:
                            setattr(
                                attr_value,
                                "embed_query",
                                create_embed_wrapper(attr_value.embed_query, attr_name),
                            )
                    ...
                    for attr_name, original_method in original_embed_methods.items():
                        if hasattr(self_obj, attr_name):
                            ...
                            try:
                                ...
                                object.__setattr__(
                                    attr_value, "embed_query", original_method
                                )
            ...
            try:
                ...
                print(
                    f"✅ {func.__name__} [{mode}] ({provider}): {call_embeddings_count} embeddings, ~{call_total_tokens} tokens"
                )
