...
class AdaptiqOfflineLearner:
    ...
    def update_policy(self, s: Tuple[str, str, str, str], a: str, R: float, s_prime: Any, actions_prime: List[str]) -> float:
        """
        ...
        max_Q_s_prime = max([self.Q(s_prime, a_prime) for a_prime in actions_prime]) if actions_prime else 0
        ...
        logger.debug(f"Formula: {Q_sa:.4f} + {self.alpha:.2f} * ({R:.2f} + {self.gamma:.2f} * {max_Q_s_prime:.4f} - {Q_sa:.4f})")
    ...
    def save_q_table(self, file_path: str, prefix_version: str= None) -> bool:
        ...
        try:
            ...
            data = {
                ...
                "version": prefix_version + "_" + str(uuid.uuid4()).replace("-", "")[:5] if prefix_version else "1.0"
            }
        ...
        try:
            ...
            for state_str, actions in serialized_q.items():
                ...
                try:
                    ...
                    logger.warning(f"Skipping state due to parsing error: {state_str}, Error: {str(e)}")
                    continue
