...
class AdaptiqHypotheticalStateGenerator:
    ...
    def __init__(self, prompt_parsed_plan: List[Dict], model_name: str, api_key: str, provider: str):
        """
    ...
    def generate_hypothetical_state_action_pairs(self) -> List[Dict]:
        ...
        context = {
            "parsed_plan": self.prompt_parsed_plan
        }
...
class AdaptiqScenarioSimulator:
    ...
    def __init__(self,  hypothetical_states: List[Dict], model_name: str, api_key: str, provider: str, output_path: str):
        """
        ...
        if self.provider == "openai":
            self.scenario_generation_llm = ChatOpenAI(temperature=0.2, model=model_name, api_key=api_key)
        else:
            raise ValueError(f"Unsupported provider: {self.provider}. Only 'openai' is currently supported.")
    ...
    @instrumental_track_tokens(mode="pre_run", provider="openai")
    def _invoke_llm_for_scenario_generation(self, state_str: str, intended_action: str, step_details: Dict) -> List[Dict]:
        """
        ...
        try:
            ...
            # Create the prompt
            scenario_prompt = self.scenario_generation_prompt_template.format_messages(**prompt_inputs)
    ...
    def generate_simulated_scenarios(self) -> List[Dict]:
        ...
        if hasattr(self, 'output_path') and self.output_path:
            try:
                ...
                print(f"Successfully saved {len(all_simulated_steps)} simulated scenarios to {self.output_path}")
            except Exception as e:
