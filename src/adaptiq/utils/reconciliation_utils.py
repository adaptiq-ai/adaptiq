...
class AdaptiqQtablePostrunUpdate:
    ...
    def __init__(self, provider: str, api_key: str, model: str = "text-embedding-3-small", alpha=0.8, gamma=0.8, similarity_threshold=0.7):
        """
        ...
        for state_str, actions in serialized_q.items():
            try:
                ...
                if state_key is None: # Should not happen with proper parsing
                    logger.warning(f"Parsed state_key is None for input {state_str}, skipping.")
                    continue
                ...
                logger.warning(f"Error processing Q-table entry for state string {state_str}: {str(e)}")
        ...
        for state_str in seen_states_list:
            try:
                ...
                if state_key is None:
                    logger.warning(f"Parsed state_key for seen_state is None for input {state_str}, skipping.")
                    continue
                self.learner.seen_states.add(state_key)
            except Exception as e:
                logger.warning(f"Error adding seen state from string {state_str}: {str(e)}")
                
        logger.info(f"Loaded Q-table with {len(self.learner.Q_table)} state-action pairs")
        ...
        try:
            ...
            def _make_hashable_recursive(item: Any) -> Any:
                if isinstance(item, list):
                    return tuple(_make_hashable_recursive(sub_item) for sub_item in item)
                elif isinstance(item, tuple):
                    return tuple(_make_hashable_recursive(sub_item) for sub_item in item)
                elif isinstance(item, dict):
                    # Sort by key to ensure consistent hashing for equivalent dicts
                    # Recursively make values hashable
                    return tuple(sorted((k, _make_hashable_recursive(v)) for k, v in item.items()))
            ...
            if isinstance(state_input, str):
                ...
                if (stripped_input.startswith("[") and stripped_input.endswith("]")) or \
                   (stripped_input.startswith("(") and stripped_input.endswith(")")):
                    try:
                        ...
                        logger.debug(f"ast.literal_eval failed for {stripped_input}: {eval_err}. Treating as plain string.")
            ...
            logger.error(f"CRITICAL Error parsing state key {state_input!r}: {str(e)}. Falling back to string representation of input.")
    ...
    def _calculate_action_similarity(self, input_action: str, q_table_actions: List[str]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if input_action in q_table_actions:
                return {action: (1.0 if action == input_action else 0.0) for action in q_table_actions}
            
            input_embedding = self.embeddings.embed_query(input_action)
            if input_embedding is None: # Embedding failed
                logger.error(f"Failed to get embedding for input_action: {input_action}")
            ...
            for action in q_table_actions:
                ...
                if emb is not None:
                    ...
                    logger.warning(f"Failed to get embedding for Q-table action: {action}")
            ...
            if input_norm == 0: # Avoid division by zero
                logger.warning(f"Input action embedding norm is zero for: {input_action}")
                return {action: 0.0 for action in q_table_actions}
            ...
            for action, embedding in action_embeddings.items():
                ...
                if embedding_norm == 0: # Avoid division by zero
                    ...
                    logger.warning(f"Q-table action embedding norm is zero for: {action}")
                    continue
                
                similarity = np.dot(input_embedding, embedding) / (input_norm * embedding_norm)
                similarities[action] = float(similarity)
            
            return similarities
        except Exception as e:
            logger.error(f"Error calculating action similarity for input '{input_action}' against {q_table_actions}: {str(e)}")
            return {action: 1.0 if action == input_action else 0.0 for action in q_table_actions}
    
    def update_q_table(self, state_classifications: List[Dict], reward_execs: List[Dict]) -> Dict:
        """
        ...
        for i, classification in enumerate(state_classifications):
            if i >= len(reward_execs):
                logger.warning(f"No reward execution data for classification at index {i}")
                continue
            ...
            if classification["classification"]["is_known_state"]:
                ...
                if not available_actions:
                    ...
                    logger.info(f"State {matched_state} has no actions in Q-table. Adding new action {input_action}.")
                    self.learner.Q_table[(matched_state, input_action)] = reward # Initial Q-value set to reward
                else:
                    if input_action in available_actions:
                        logger.info(f"Exact action match. Updating Q-value for state {matched_state}, action {input_action}")
                        next_state_parsed = None
                        next_actions_for_next_state = []
                        if i + 1 < len(state_classifications) and state_classifications[i + 1]["classification"]["is_known_state"]:
                            ...
                            next_actions_for_next_state = self._get_actions_for_state(next_state_parsed)
                        ...
                        similarities = self._calculate_action_similarity(input_action, available_actions)
                        if not similarities: # No similar actions could be computed or no actions available
                             logger.info(f"No similar actions found or could be computed for {input_action} in state {matched_state}. Adding as new action.")
                             self.learner.Q_table[(matched_state, input_action)] = reward
                             continue # Skip to next classification

                        most_similar_action_tuple = max(similarities.items(), key=lambda x: x[1])
                        
                        if most_similar_action_tuple[1] >= self.similarity_threshold:
                            action_to_update = most_similar_action_tuple[0]
                            logger.info(f"Similar action found: '{action_to_update}' (sim: {most_similar_action_tuple[1]:.2f}) for input '{input_action}'. Updating Q-value for state {matched_state}.")
                            ...
                            logger.info(f"No action similar enough (max sim: {most_similar_action_tuple[1]:.2f} < {self.similarity_threshold}) for '{input_action}'. Adding as new action to state {matched_state}.")
                            self.learner.Q_table[(matched_state, input_action)] = reward
            else:
                logger.info(f"Skipping unknown state at index {i}: {classification['input_state']}")
        
        output_q_table = {}
        for (state, action), value in self.learner.Q_table.items():
            state_key_str = str(state) # Convert hashable state back to string for JSON output
    ...
    def process_data(self, state_classifications_data: List[Dict], reward_execs_data: List[Dict], q_table_data: Dict) -> Dict:
        """
        ...
        updated_q_table = self.update_q_table(state_classifications_data, reward_execs_data)
        self.learner.save_q_table(file_path="adaptiq_q_table.json", prefix_version="post_run")
...
class AdaptiqPromptEngineer:
    def __init__(self, main_config_path: str):
        ...
        try:
            if self.provider == "openai":
                self.llm = ChatOpenAI(model=self.model_name, api_key=self.api_key, temperature=0.3)
            else:
                raise ValueError(f"Unsupported provider: {self.provider}. Only 'openai' is currently supported.")

        except Exception as e:
            logger.error(f"Failed to initialize ChatOpenAI: {e}. Ensure API key is valid.")
    ...
    def _load_old_prompt(self) -> str:
        ...
        """
        prompt_file_path_str = self.agent_modifiable_config.get("prompt_configuration_file_path")
        ...
        if not os.path.exists(prompt_file_path_str):
            logger.error(f"Agent prompt configuration file not found: {prompt_file_path_str}")
            raise FileNotFoundError(f"Agent prompt configuration file not found: {prompt_file_path_str}")
        ...
        for key, config in prompts_config.items():
            if isinstance(config, dict) and "description" in config:
                ...
                logger.info(f"Successfully loaded old prompt for task: {self.task_name}")
                return old_prompt.strip()

        logger.error("No task with a 'description' field found in prompt configuration file.")
        raise ValueError("No task with a 'description' field found in prompt configuration file.")
    ...
    def _invoke_llm_for_analysis(self, old_prompt: str, q_table_insights: str) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            return f"Error during LLM invocation: {e}", f"Error during LLM invocation for review: {e}"

        # This is a simple split; more robust parsing might be needed if LLM format varies
        suggested_prompt_header = f"## Suggested Enhanced Prompt for Task '{self.task_name}'"
        
        parts = full_response_text.split(suggested_prompt_header, 1)
        
        review_and_diagnostic = parts[0].replace("## Agent Review and Diagnostic", "").strip()
        ...
        if len(parts) > 1:
            ...
            if "```" in suggested_new_prompt:
                ...
                if len(code_block_content) > 1: # Found at least one ```
                    ...
                    if len(lines) > 1 and not lines[0].strip().isalnum() and len(lines[0].strip()) > 0: # e.g. not 'yaml' but some junk
                        suggested_new_prompt = potential_prompt.strip()
                    elif len(lines) > 1 and lines[0].strip().isalnum(): # e.g. 'yaml' or 'text'
            ...
            logger.warning("Could not clearly separate suggested prompt from LLM response.")
    ...
    def _save_report(self, report_content: str, agent_name_for_report: str):
        ...
        """
        output_path_template = self.report_config.get("output_path", "reports/agent_report.md")
        
        # Substitute agent name if placeholder is present
        if "your_agent_name" in output_path_template and agent_name_for_report:
            output_path = output_path_template.replace("your_agent_name", agent_name_for_report.replace(" ", "_"))
    ...
    def generate_and_save_report(self, q_table_output: Dict) -> str:
        ...
        suggested_new_prompt, review_diagnostic = self._invoke_llm_for_analysis(old_prompt, q_insights)
        ...
        report_content += "```\n\n"
        report_content += f"## Suggested Enhanced Prompt for Task '{self.task_name}'\n\n"
